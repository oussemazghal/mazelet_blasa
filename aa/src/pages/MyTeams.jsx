...
export default function MyTeams() {
    ...
    async function handleDeleteTeam(teamId, teamName) {
        if (!window.confirm(`⚠️ Are you sure you want to delete "${teamName}"? This action cannot be undone.`)) return;

        const token = localStorage.getItem("token");
        try {
            const res = await fetch(`http://127.0.0.1:8001/teams/${teamId}`, {
                method: "DELETE",
                headers: {
                    "Authorization": `Bearer ${token}`
                }
            ...
            if (res.ok) {
                ...
                const err = await res.json();
                showToast("Error: " + (err.detail || "Failed to delete team"), "error");
            }
        } catch (error) {
            console.error(error);
            showToast("Network error", "error");
            ...
            <div className="teams-grid">
                ...
                {teams.map(team => (
                    <div key={team.id} className="team-card">
                            ...
                            <p>
                                ...
                                {user && team.captain_id === user.id ? (
                                    ...
                                    <span
                                        ...
                                        onClick={() => navigate(`/profile/${team.captain_id}`)}
                                    >
                                        {team.members.find(m => m.user_id === team.captain_id)?.name || "Unknown"}
                                    </span>
                                )}
                            </p>
                        ...
                        {/* Delete button for captain only */}
                        {user && team.captain_id === user.id && (
                            <button
