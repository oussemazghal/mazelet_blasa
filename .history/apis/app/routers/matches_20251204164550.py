...
def create_match(match: schemas.MatchCreate, db: Session = Depends(database.get_db), current_user: models.User = Depends(auth.get_current_user)):
    ...
    if new_match.is_team_match:
        ...
        # Validate team size matches match type
        required_size = int(new_match.type_match.replace("v", "").split("v")[0])
        team_size = len(team.members) if team.members else 0
        
        if team_size != required_size:
            raise HTTPException(
                status_code=400, 
                detail=f"Your team must have exactly {required_size} members for a {new_match.type_match} match! Currently you have {team_size} members."
            )
...
def join_match(
    ...
    if match.is_team_match:
        ...
        # Validate team size matches match type
        required_size = int(match.type_match.replace("v", "").split("v")[0])
        team_size = len(team.members) if team.members else 0
        
        if team_size != required_size:
            raise HTTPException(
                status_code=400, 
                detail=f"Your team must have exactly {required_size} members for a {match.type_match} match! Currently you have {team_size} members."
            )
...
@router.delete("/{match_id}/remove-team-b")
def remove_team_b(
    match_id: int,
    db: Session = Depends(database.get_db),
    current_user: models.User = Depends(auth.get_current_user)
    ...
    # Check if current user is the organizer
    if match.organizer_id != current_user.id:
        raise HTTPException(status_code=403, detail="Only the organizer can remove teams")
    
    # Check if it's a team match
    if not match.is_team_match:
        raise HTTPException(status_code=400, detail="This is not a team match")
    
    # Check if Team B exists
    if not match.team_b_id:
        raise HTTPException(status_code=400, detail="Team B has not joined yet")
    
    # Get Team B members to remove from participants
    ...
    # Remove all Team B members from participants
    for member in team_b_members:
        user = db.query(models.User).filter(models.User.id == member.user_id).first()
        if user and user in match.participants:
    ...
    return {"message": "Team B removed successfully"}
