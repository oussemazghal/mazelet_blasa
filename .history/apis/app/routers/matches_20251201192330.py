...
def create_match(match: schemas.MatchCreate, db: Session = Depends(database.get_db), current_user: models.User = Depends(auth.get_current_user)):
    ...
    if new_match.is_team_match:
        ...
        # We don't necessarily add members to participants for Team Match, 
        # but we could to make "spots left" logic work if we want.
        # However, for Team vs Team, it's 1 vs 1 (Team A vs Team B).
        # Let's assume nb_players in Team Match context might mean "Players per team" or just ignore it?
        # Let's keep it simple: Team Match = 2 slots (Team A and Team B).
        # But wait, the user wants "Find a Counter".
    else:
        # Normal Match Logic
