...
export default function MyOrganizedGames() {
    ...
    async function handleRemoveTeam(matchId) {
        if (!window.confirm("Are you sure you want to remove Team B from this match?")) return;

        const token = localStorage.getItem("token");
        try {
            const res = await fetch(`http://127.0.0.1:8001/matches/${matchId}/remove-team-b`, {
                method: "DELETE",
                headers: {
                    "Authorization": `Bearer ${token}`
                }
            ...
            if (res.ok) {
                showToast("Team B removed successfully", "success");
                fetchMyOrganizedGames(); // Refresh list
            } else {
                const err = await res.json();
                        ...
                        <div key={match.id} className="game-card" style={{ borderColor: "#2196F3", cursor: "default" }}>
                            ...
                            <div style={{ marginTop: "15px", borderTop: "1px solid #eee", paddingTop: "10px" }}>
                                {match.is_team_match ? (
                                    <>
                                        <strong>Teams:</strong>
                                        <div style={{ marginTop: "10px", background: "#f0f4c3", padding: "10px", borderRadius: "8px" }}>
                                            <p><strong>Team A:</strong> {match.team_a?.name || "Unknown"}</p>
                                            <p><strong>VS</strong></p>
                                            <p><strong>Team B:</strong> {match.team_b?.name || "Waiting for opponent..."}</p>
                                            {match.team_b && (
                                                <button
                                                    onClick={() => handleRemoveTeam(match.id)}
                                                    style={{
                                                        background: "#ff4444",
                                                        color: "white",
                                                        border: "none",
