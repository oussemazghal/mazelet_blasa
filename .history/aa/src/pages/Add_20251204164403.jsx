...
export default function CreateMatch() {
  ...
  async function handleCreateMatch() {
    ...
    // Team Size Validation for Team vs Team
    if (matchMode === "team_vs_team") {
      ...
      const requiredSize = parseInt(formData.typeMatch.replace("v", "").split("v")[0]);
      const teamSize = selectedTeam.members ? selectedTeam.members.length : 0;

      if (teamSize !== requiredSize) {
        showToast(`Your team must have exactly ${requiredSize} members for a ${formData.typeMatch} match! Currently you have ${teamSize} members.`, "error");
        return;
      }
    }
    ...
    const matchData = {
      ...
      nb_players: matchMode === "team_vs_team" ? 0 : parseInt(formData.nbPlayers),
