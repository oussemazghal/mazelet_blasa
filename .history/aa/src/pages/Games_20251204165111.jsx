...
export default function Games() {
  ...
  useEffect(function () {
    async function loadData() {
      try {
        // 1. Fetch Matches with upcoming_only=true and exclude_full_team_matches=true
        const resMatches = await fetch("http://127.0.0.1:8001/matches/?upcoming_only=true&exclude_full_team_matches=true");
  ...
  async function handleJoinAsTeam() {
    ...
    // Find the match and selected team
    ...
    if (!match || !selectedTeam) {
      showToast("Match or team not found", "error");
      return;
    }

    // Calculate required team size based on match type
    const requiredSize = parseInt(match.type_match.replace("v", "").split("v")[0]);
    const teamSize = selectedTeam.members ? selectedTeam.members.length : 0;

    if (teamSize !== requiredSize) {
      showToast(`Your team must have exactly ${requiredSize} members for a ${match.type_match} match! Currently you have ${teamSize} members.`, "error");
      return;
    }
  ...
  const renderMatchCard = (match, isRecommendation = false, recData = null) => {
    ...
    return (
      <div key={match.id} className="game-card" style={isRecommendation ? { border: "2px solid #667eea", position: "relative" } : {}}>
        ...
        {match.is_team_match ? (
          <div style={{ margin: "10px 0", padding: "10px", background: "#f0f4c3", borderRadius: "8px" }}>
            <p><strong>Team A:</strong> {match.team_a ? match.team_a.name : "Unknown"}</p>
            <p><strong>VS</strong></p>
            <p><strong>Team B:</strong> {match.team_b ? match.team_b.name : "Waiting for opponent..."}</p>
          </div>
        ) : (
          <p>
              ...
              {match.is_team_match ? (
                <>
                  ...
                  <p style={{ fontSize: "13px", marginBottom: "8px" }}>
                    ...
                    <span
                      style={{ color: "#2196F3", cursor: "pointer", textDecoration: "underline" }}
                      onClick={() => navigate(`/profile/${match.team_a?.captain_id}`)}
                    >
                      {match.team_a?.members?.find(m => m.user_id === match.team_a.captain_id)?.name || "Unknown"}
                    </span>
                  </p>
                  ...
                  {match.team_b && (
                    <>
                      ...
                      <p style={{ fontSize: "13px", marginBottom: "8px" }}>
                        ...
                        <span
                          style={{ color: "#2196F3", cursor: "pointer", textDecoration: "underline" }}
                          onClick={() => navigate(`/profile/${match.team_b?.captain_id}`)}
                        >
                          {match.team_b?.members?.find(m => m.user_id === match.team_b.captain_id)?.name || "Unknown"}
                        </span>
                      </p>
      ...
      <div className="games-list">
        {showRecommended ? (
          recommendations.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px", background: "rgba(255, 255, 255, 0.9)", borderRadius: "12px", gridColumn: "1 / -1" }}>
              <p style={{ color: "#666", fontSize: "16px" }}>Aucune recommandation disponible. Participez à des matchs pour obtenir des recommandations personnalisées !</p>
            </div>
          ) : (
          ...
          filteredGames.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px", background: "rgba(255, 255, 255, 0.9)", borderRadius: "12px", gridColumn: "1 / -1" }}>
              <p style={{ color: "#666", fontSize: "16px" }}>Aucun match trouvé avec ces filtres.</p>
            </div>
          ) : (
